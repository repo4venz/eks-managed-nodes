...
resource "aws_eks_node_group" "demo_eks_nodegroup_ondemand_high_pod" {
  ...
  node_group_name = "${var.cluster_name}-${var.environment}-${replace(each.key, ".", "")}-nodes-group-ondemand-high-pods" 
  ...
  depends_on = [
    ...
    aws_launch_template.eks_worker_nodes_ondemand_high_pod
